...
(function($) {
    window.VtexSearch = {
        ...
        _search: function(method, callback, attempts) {
            ...
            console.log('busca pagina:  ', self.request.url);
        ...
        _getTotalItems: function(callback, attempts) {
            ...
            if (typeof callback === 'function') {
                ...
                $.ajax({
                    ...
                    if (error.status === 500 && attempts < self.options.attempts) {
                        attempts++;
                        self._getTotalItems(callback, attempts);
                    }

                    throw new Error('Error on get total items', error);
                });
        ...
        _splitRequestUrl: function(url) {
            ...
            if (splitUrl.length > 1) {
                ...
                console.log('LOCALIZAÇÃO ESTA AQUI ' + location)
                var search = location.search;
                console.log('PESQUISA ESTA AQUI ' + search)
        ...
        bindFilters: function() {
            ...
            self.options.$filters.on('click', function(event) {
                 ...
                 * **************
                 */
                /* if (window.location.href.indexOf('vtex') > -1 && $('body').hasClass('stable')) { */
                var filter = $checkbox.attr('rel').replaceAll('+', '%20'); //Retira o sinal de + e inseri %20
                /*   } else {
                      var filter = $checkbox.attr('rel');
                  } */
                /*  } else { */
                /*  var filter = $checkbox.attr('rel'); */
                /*  } */
                console.log('FILTRO ESTA AQUI ' + filter)
        ...
        _refreshFilter: function(filter, action, _this) {
            ...
            self._loadFirst(function() {
                ...
                console.log('HASH AQUI TBM ' + self._setUrlHash(1))
                self._setUrlHash(1);
